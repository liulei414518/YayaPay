...
base64decode(des(key,hexToString(param),0,0))//解密1

使用方法2：
base64encode(des(key,message,1,1, hexToString(value), 1));//加密2
des(key,base64decode(String(param)),0,1,hexToString(value),0)//解密2
*/

import React from 'react';

class DES3 extends React.Component{


    triple_des(key, message) {  
        for (var i = key.length; i<24; i++) {  
            key+="0";  
        }  
        return this.des(key, message, 1, 0, 0, 1);  
    }
